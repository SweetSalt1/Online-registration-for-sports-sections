...
void UI::MenuGuest() {
    ...
    if (x == 1) {
        ...
        if (api_.Register(l, p)) std::cout << "OK!\n";
    ...
    if (x == 2) {
        ...
        if (api_.Login(l, p)) std::cout << "Logged!\n";
...
void UI::MenuStudent() {
    ...
    if (x == 2) {
        ...
        std::cout << "Text: "; std::cin.ignore();
        std::getline(std::cin, text);

        if (api_.SendApplication(id, text)) std::cout << "Sent\n";
        else std::cout << "Failed\n";
    }
}
...
void UI::MenuSecretary() {
    ...
    if (x == 2) {
        ...
        std::cout << "1 approve / 0 reject: "; std::cin >> ok;

        if (api_.Moderate(id, ok)) std::cout << "Done\n";
        else std::cout << "Failed\n";
    }
}
